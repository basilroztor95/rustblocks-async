    ...
    pub fn run(&mut self) -> types::Result<()> {
        ...
        while let Ok(indexes) = receiver.recv() {
            ...
            for (index, child) in spawns {
              ...
              //new_cells[index] = block.print(String::from_utf8_lossy(output.stdout.as_slice()).to_string());
                new_cells[index] = block.print(String::from_utf8_lossy(output.stdout.as_slice()).trim().to_string());
